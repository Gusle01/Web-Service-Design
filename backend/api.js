...
const corsOptions = {
  ...
  origin: "http://113.198.66.75:10127", // JCloud 배포 주소
...
const posts = []; // { id, title, category, username, desc, regdate, thumbnail, authorEmail }
...
// POST /posts: 새 게시글 작성 (인증 필요)
router.post("/posts", authenticateToken, (req, res) => {
  const { title, category, desc, username, thumbnail } = req.body;

  if (!title || !category || !desc || !username || !thumbnail) {
...
router.delete("/posts/:id", authenticateToken, (req, res) => {
  ...
  if (posts[postIndex].authorEmail !== req.user.email) {
    return res
      .status(403)
      .json({ message: "게시글은 작성자만 삭제할 수 있습니다." });
  }
